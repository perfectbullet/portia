...
def port_project(dir_name, schemas, spiders, extractors, selector='css'):
    ...
    schema_names = {}
    print('schemas is {}'.format(schemas))
    print('items is {}'.format(items))
    for _id, name in schemas.items():
        print('name.get(name, _id)  is {}'.format(name.get('name', _id)))
        print('get class_name name is {}'.format(class_name(name.get('name', _id))))
        name = '{}Item'.format(class_name(name.get('name', _id)))
        schema_names[_id] = items[name]
    schema_names['_PortiaItem'] = items['PortiaItem']

    spider_data = create_spiders(spiders, schemas, extractors, schema_names, selector)
